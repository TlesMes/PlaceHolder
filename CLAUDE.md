...
- **Phase E-2: 프론트 충전 UI** — PR #23 (`feature/payment-frontend-ui`, **머지 완료 2026-08-03**)
  ...
  - ⚠️ 실제 결제창은 아직 안 뜸(키 미발급). successUrl 콜백은 토스가 보낼 쿼리스트링을 흉내 내 검증 — 파싱→confirm→렌더 경로는 실증됐으나 실제 리다이렉트 도달은 키 확보 후 확인 필요. 결제수단 카드 고정

### 현재 상태
- **작업 브랜치:** `main` (**E-2 결제 백엔드·프론트 모두 머지 완료**, 다음 작업 미착수)
- **마지막 main 커밋:** `Merge pull request #23` (79d0840)
  - PR #1~11, #13~23 머지 완료
  - **E-2 결제 완료(정합성 축):** 백엔드(#22, 07-30, ADR-018) + 프론트 충전 UI(#23, 08-03).
    남은 건 토스 테스트 키 발급(실 샌드박스 E2E)과 생애주기 축(아래 결제 백로그)
...
### 다음 작업 (우선순위 순)
> E-2 머지로 마스터플랜 체크리스트의 **미착수 핵심 어필 항목은 모두 소진**됐다. 남은 건 완결도(키·생애주기)와 Phase F(문서 마감)·E-4(분산).
1. **토스 테스트 키 발급**(사용자 작업) → `TOSS_CLIENT_KEY`/`TOSS_SECRET_KEY` env 주입하면 실제 결제창이 뜨고 목킹을 실호출로 교체 가능 → **E-2 실 샌드박스 E2E 완결**. 코드는 이미 준비됨(가장 싼 완결).
2. **F-3 포트폴리오 문서 마감** — README 확장(domain→problem→solution→result), ADR 18종 정리, 면접 스토리 2종(동시성 설계 서사 / 정산의 법적 맥락). 인프라 의존 없고 어필 효과가 큼.
3. **결제 생애주기 착수 판단** — 아래 결제 백로그 1·2번(취소·환불, 대사)이 "포인트 충전을 구현했다"고 말하기 위한 최소 잔여분.
4. **E-4 분산 환경 동시성**(분산 락) — 마스터플랜 미착수 Phase. 멀티 인스턴스+LB 전제라 *인프라 의존*.
5. **F-1 CD**(AWS EC2 배포) — CI는 #12로 완료, 배포는 미착수. 마스터플랜상 "시간 여유 시".
6. 나머지는 대기열 백로그 — 실측 계열(2·3번)은 인프라 의존.
...
### 백로그 — 결제(E-2) 미구현 영역
> PR #22 회고에서 식별. **정합성 축**(멱등·금액 위변조·트랜잭션 경계·타임아웃)은 정석대로 채웠으나
> **생애주기 축**은 통째로 비어 있다. "포인트 충전을 구현했다"고 말할 때 함께 밝혀야 할 공백.
1. **취소·환불 부재(가장 큰 구멍)** — 토스 취소 API 호출 + 포인트 회수 경로가 0. 실서비스면 청약철회가 법적 요구라 필수. "이미 써버린 포인트는 어떻게 되나"(잔액 부족 시 회수 정책·부분 취소)라는 도메인 설계가 따라온다.
2. **대사(reconciliation) 부재** — confirm도 실패하고 웹훅도 놓친 결제는 현재 영구 미아. 토스 거래내역 ↔ 우리 DB를 주기 배치로 대조하는 이중화의 마지막 층이 빠짐.
3. **비동기 결제수단 미지원(가상계좌 등)** — 상태머신이 `READY→DONE/FAILED`뿐이라 "입금 대기"(며칠 지속) 중간 상태를 표현 못 함. **웹훅이 진짜 필요해지는 대표 사례가 이것**인데 현재 웹훅은 confirm 누락 보정 용도로만 쓰인다. 프론트도 `requestPayment('카드')` 고정.
4. **고아 주문 정리 없음** — 결제창을 닫으면 `READY` 주문이 영구 잔존. 좌석 hold 만료(ADR-009)처럼 TTL 만료 처리 필요.
5. **웹훅 서명 검증** — 현재는 재조회 검증까지(ADR-018 한계에 명시). 서명/IP 화이트리스트는 미적용.
6. **충전 한도·감사 로그·영수증** 없음. 포인트 충전은 규제상 단순 결제와 다르게 취급될 수 있음(선불 성격의 자금 보관 → 이용자 자금 보호 의무 등) — **실제 요건 확인 필요**. 프로젝트 어필 축인 "정산의 법적 맥락"과 직결되는 도메인 숙제.
