...
- **법적 맥락 조사 → 결제 취소 기한 개정** — PR #35 (`fix/payment-cancel-period`, **머지 완료 2026-08-22**, ADR-019 1-1 절 추가)
  - 마스터플랜 3대 축(동시성·프로세스 성숙도·**법적 맥락**) 중 실질 작업이 가장 적던 축을 마감. 조사의 성과는 규제 대응이 아니라 **기능 결함 1건 발견**이었다
  - **문제 — 두 개의 다른 권리를 한 기한으로 묶고 있었다:** 취소 기한 7일의 근거가 "전자상거래법 청약철회 기간"이었는데, 청약철회권("거래를 물릴 권리", 7일)과 **미사용 잔액 환급권**("안 쓴 돈을 돌려받을 권리", 통상 소멸시효 전 5년)은 별개다. 출금(`PAYOUT`) 경로가 없어 결제 취소가 **현금을 되찾는 유일한 통로**라, 7일 컷오프는 실질적으로 **"8일째부터 미사용 현금 영구 인출 불가"** 였다
  - **5년이 아니라 1년인 이유는 결제망 제약(작성자 지적).** 토스 문서상 카드는 명시적 취소 기한이 없으나 카드사 데이터 보관 기간을 넘기면 취소 자체가 실패한다 → **지키지 못할 기한을 약속하는 것은 7일 컷오프와 방향만 다른 같은 문제**라 실제 이행 가능한 상한을 택함. `period-days: 7 → 365`
  - **범위 밖으로 확정(미루는 게 아니라 안 함):** 1년 초과 미사용 잔액은 그대로 둔다(쓰거나 소멸). 출금·환급률(90%) 정책은 **선불업 등록·선불충전금 별도 관리 같은 실서비스 사업자 의무 영역**이라 이 프로젝트가 따라갈 축이 아니다. 규제 자체는 등록 면제 구간(발행잔액 30억/연 500억 미만)
  - **함께 고친 테스트 결함:** 경계 테스트가 기한을 `7 * 24`로 하드코딩 → 설정만 바뀌면 조용히 무의미해지는 구조(PR #26 "거부를 기대하는 테스트는 빗나가도 통과"와 동종). `@Value` 주입으로 교체
  - **음성 대조:** 기한을 7로 되돌려도 경계 테스트 통과 → **서로 다른 두 기한(7·365)에서 각각 "마감 1시간 전 성공 / 1시간 후 거부" 재현** = 게이트가 상수가 아니라 `승인시각 + 설정기한`에서 실제로 전환됨을 확인
  - ⚠️ 우려했던 **무단 수수료 차감류는 애초에 없음** — 환불액은 `min(주문 잔여액, 유료 잔액)`뿐이고 퍼센트 차감 로직이 코드에 존재하지 않는다. 예약 취소·위약금도 **기능 자체가 없어** 위반 지점 없음(단, 향후 예약 취소를 만든다면 소비자분쟁해결기준의 날짜별 위약금률 참고 필요)
  - 테스트 208 통과(신규 없음, 동작 경계만 이동). **이로써 마스터플랜 어필 체크리스트 11개 전부 완료**

- **E-4 실검증 + 대사 한도 정리** — PR #36 (`test/reconciliation-duplicate-run`, **머지 완료 2026-08-22**)
  - E-4("멀티 인스턴스에서 버티는가") 4갈래 중 **실제로 검증이 필요했던 1갈래만 떼어 닫았다.** 나머지는 코드 논증·기존 문서로 종결(위 "다음 작업" 5번)
  - **비어 있던 축:** 대사 경합 테스트가 `대사×웹훅`·`대사×사용자취소`는 덮었으나 **`대사×대사`(인스턴스 2대가 같은 잡을 동시 실행)** 는 없었다. `ReconciliationDuplicateRunTest` 신규 10건(정방향·역방향 각 `@RepeatedTest(5)`)
  - **실 인스턴스 2개를 띄우지 않은 이유:** 검증 대상이 프로세스 경계가 아니라 **같은 DB 행을 동시에 건드리는 두 실행**이다. 스레드 2개로 재현되고, 래치·배리어로 겹치는 순간을 강제할 수 있어 오히려 확실하다. 이 경로의 정합성 주체는 DB 락·멱등 코어이지 앱 메모리가 아니다
  - **위양성 차단(설계상 핵심):** 한쪽이 먼저 끝나면 다른 쪽은 후보를 못 봐 "동시 실행"이 아니라 이미 증명된 순차 멱등성만 재확인하게 된다 → **`CyclicBarrier`로 둘 다 후보를 집은 뒤에야 진행**시키고, `verify(times(2)).findByOrderId`로 **경합이 실제 일어났음을 단정**
  - **⚠️ 음성 대조에서 알게 된 것 — 방어선이 두 겹이었다:** 서비스 멱등 판정(`settle`의 `isSettled`)만 끄자 **테스트가 그대로 통과했다.** 엔티티 `markDone`이 READY가 아니면 예외를 던져 막고 있었기 때문. **둘 다 꺼야 20,000으로 갈리며 5/5 실패** — 이 테스트가 이중 적립을 실제로 잡는다는 근거이자, 가드 하나가 사라져도 돈이 새지 않는다는 확인. 성격도 다르다(서비스=조용한 no-op·정상 경로 / 엔티티=시끄러운 예외·도달 금지 전이)
  - 대사 batch-size를 **호출 수 기준으로 재산정**(100/100 → 30/20, 최악 70콜 < 분당 100건). 결제 백로그 8번 해소
  - PR #35에서 놓친 stale javadoc 1건 수정(`PaymentOrder.isWithinCancelPeriod`가 아직 "청약철회 기간"이라고 설명)
  - 테스트 218 통과(기존 208 + 10). 프로덕션 코드 변경 없음(설정·주석만)

### 현재 상태
- **작업 브랜치:** `main` (**계획된 작업 종결.** 어필 체크리스트 11개 확보 + E-4 마감. 지금 환경에서 완결 가능한 항목은 소진됨 — 남은 건 배포(F-1)와 인프라 의존 실측뿐)
- **마지막 main 커밋:** `Merge pull request #36` (7fe53d2)
  - PR #1–11, #13–36 머지 완료
...
- **실행 가능 API:**
  ...
  - **POST /api/payments/{orderId}/cancel** - 결제 취소·환불(미사용분 부분 취소, **승인 후 1년 이내**, 멱등) (BOOKER 본인) ★ E-2
...
> 마지막 남았던 법적 맥락 축은 #35로 닫혔다(조사가 실제 기능 결함 1건을 찾아냈다).
> E-4는 네 갈래로 쪼개 실검증이 필요한 하나만 #36으로 닫았다.
> **알려진 정합성 결함·미복구 경로가 없고, 어필 체크리스트 11개가 전부 확보됐다 = 계획된 작업 종결.**
> **이 PC 환경에서 완결 가능한 항목은 소진됐다.** 남은 것은 전부 외부 조건이 필요하다 —
> F-1 CD(배포 대상 필요, 오라클 무료 계정 보유), 실측 계열(부하생성기 별도 머신),
> 토스 SDK v2·웹훅 서명(계약·공개 URL). 트리거가 생기면 착수한다.
...
4. ~~**법적 맥락 조사**~~ → **PR #35 머지 완료 2026-08-22.** 취소 기한 7일이 청약철회권과 미사용 잔액 환급권을 뒤섞어 "미사용 현금 영구 인출 불가"를 만들던 결함 수정(→ 1년, 결제망 상한). **이로써 어필 체크리스트 11개 전부 완료 — 계획된 작업이 끝났다.**
5. **E-4 분산 환경 동시성**(분산 락) — **검토 후 종결 처리 2026-08-22.** ①입장 상한은 `admit.lua` 원자성이라 인스턴스 수와 무관(코드 논증으로 종결) ②deficit 장부 인메모리 한계는 이미 알고 문서화됨(ADR-017, 재확인해도 새 정보 없음) ③**대사 잡 중복 실행 = 유일한 실검증 대상 → PR #36으로 닫음** ④DB락 vs Redis락 비교는 Redisson 의존성부터 없어 사실상 **새 기능 개발 + ADR**이라 "검증"이 아님. **남은 건 ④뿐이고, 그건 필요가 생기면 착수할 별개 기능이다**
6. **F-1 CD**(배포) — CI는 #12로 완료, 배포는 미착수. 마스터플랜상 "시간 여유 시". **오라클 클라우드 무료 계정 보유**(2026-08-22 확인) — EC2보다 Docker Compose를 통째로 올리기에 이쪽이 맞다. 순서는 `Dockerfile 작성 → compose에 app 추가 → 수동 배포 1회 → 자동화`. ⚠️ 착수 판단 시 고려: 배포하면 **토스 웹훅이 처음으로 진짜 열린다**(공개 URL 필요, 지금까지 동기 승인+대사로만 검증)는 게 이득이고, **프리티어 만료 후 죽은 링크가 미배포보다 나쁘다**는 게 위험이다
7. 나머지는 결제/대기열/기타 백로그 — 실측 계열은 *인프라 의존*(부하생성기 별도 머신 필요).
...
8. ~~**대사 batch-size vs 토스 API 제한**~~ → **해소 2026-08-22 (PR #36).** 두 대사 잡이 각각 100이라 같은 분에 겹치면 한도(분당 100건)를 넘길 수 있었다. **호출 수 기준으로 재산정** — 정방향은 주문당 1콜이라 30, 역방향은 조회+취소 2콜이라 20 → 최악 70콜 < 100. **대량 누락 상황(= 대사가 가장 필요한 순간)에 한도를 소진하면 복구가 되레 막힌다**는 게 요점.
