...
### 현재 상태
- **작업 브랜치:** `main` (**E-2 결제 생애주기까지 머지 완료**, 다음 작업 미착수)
- **마지막 main 커밋:** `Merge pull request #29` (9d4080a)
  - PR #1~11, #13~29 머지 완료
  - **E-2 결제 완료:** 백엔드(#22, 07-30, ADR-018) + 프론트 충전 UI(#23, 08-03) + 대사·실 토스 E2E(#24·#25, 08-04) + **취소·환불(#26, 08-19, ADR-019)**.
    정합성 축(멱등·위변조·트랜잭션 경계·타임아웃) + 삼중화(동기·웹훅·대사) + **생애주기 축(취소·환불 보상 트랜잭션)**까지 완료.
    **실 토스 결제·취소 양방향 관통 확인**(테스트 키). 남은 건 역방향 대사(아래 결제 백로그 2번)와 환불 재원 구분
...
### 다음 작업 (우선순위 순)
> #26이 드러낸 금전적 구멍은 #27로, ADR-020 검토 중 발견한 정산액 lost update는 #28로 봉쇄됐다.
> **알려진 정합성 결함은 현재 없다.** 남은 것은 복구 장치(역방향 대사), 측정 한 건, 그리고 문서다.
> 마스터플랜 어필 체크리스트 11개 중 미확보는 F-3의 "정산의 법적 맥락·README" 하나뿐 —
> **기술 축은 사실상 다 쌓였다.**
1. **역방향 대사 (PR-B)** — #26이 만든 크래시 창(①포인트 회수 커밋 후 ②토스 취소 호출 전 사망 → 포인트만 회수되고 환불 안 됨)을 메운다. 후보는 `cancel_confirmed_at IS NULL`인 취소 주문 = "다시 보내야 할 요청". 멱등 키(누적 취소액 기준)가 있어 재시도가 안전. **#27 E2E에서 두 시각이 1.3초 벌어지는 것을 실제로 관측했다 — 그 사이가 창이다.**
2. **F-3 포트폴리오 문서 마감** — README 확장(domain→problem→solution→result), ADR 20종 정리, 면접 스토리 2종(동시성 설계 서사 / 정산의 법적 맥락). **인프라 의존 없음.** ⚠️ 면접·제출이 가깝다면 1번보다 이쪽을 먼저 — 기술 축은 충분하고 비어 있는 건 문서다.
   - **면접 스토리 소재 — "SETTLE 예외에서 원장 모델까지" (ADR-020 한계 1):** 불변식 `amount = 버킷합`을 테이블 전체에 걸었더니 버킷합 0인 `SETTLE` 행이 거부돼 **좌석 확정이 통째로 롤백**되는 것을 계획 단계에서 발견 → 타입으로 범위를 좁혀 해결. 그런데 **"타입으로 한정해야 했다는 사실 자체가 신호"** 였다. 파고들면 `point_transactions`가 예약자·제공자 두 원장을 겸직하고, 행이 "누가 얼마"만 담을 뿐 **"어느 계정에서 어느 계정으로"라는 가치 이동의 양끝이 없다**는 공통 원인이 나온다(충전 건별 귀속 부재도 같은 뿌리). **정석은 타입 분리가 아니라 계정 축 + 대차평형** — 그 모델에선 `거래 내 합 = 0` 하나로 통일되어 SETTLE 예외가 구조적으로 사라진다. **비용 재고 후 미룬 것이 결론**(잔액·이력 모두 정확하고 검증됐으며, 바꾸면 C-2 3번째 수정 + 이력/정산 조회 재작성 → **버그가 아니라 표현력 한계**). 갚을 신호 3가지를 명시해뒀다. → *증상에서 원인으로 올라가되 비용을 근거로 멈춘 판단*의 사례.
3. **제공자 정산 처리량 측정** — #28이 락으로 정합성을 확보하며 **같은 제공자의 판매가 계정 행 하나에서 직렬화**되는 상한을 남겼다. **상한 존재 ≠ 병목**이라 측정이 먼저다. 병목이면 `SETTLE` INSERT만 하고 잔액을 원장에서 파생(+스냅샷 배치)으로 재설계. 계획·측정 설계·DB vs Redis 평가는 `docs/performance/provider-settlement-throughput.md`. *인프라 의존(부하 환경).* 재설계까지 가면 **D-1(N+1)에 이어 측정이 설계를 바꾼 두 번째 사례**라 F-3 서사 소재가 된다.
...
2. **역방향 대사 (→ "다음 작업" 2번)** — #26으로 취소가 생기면서 **비로소 실제 발생 가능해졌다.** ①포인트 회수 커밋 후 ②토스 취소 호출 전 크래시 시 "우리는 취소, 토스는 승인"이 남는다. `cancel_confirmed_at IS NULL`인 취소 주문을 배치가 재시도한다(`Idempotency-Key` 보유로 재시도 안전).
