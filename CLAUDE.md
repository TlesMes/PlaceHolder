...
- **제공자 정산액 lost update 차단** — `fix/provider-settlement-lost-update` (**작업 중 2026-08-21**)
...
- **마지막 main 커밋:** `Merge pull request #27` (6affa1a)
  - PR #1~11, #13~27 머지 완료
...
### 다음 작업 (우선순위 순)
> #26(취소·환불)이 드러낸 금전적 구멍은 #27로 봉쇄됐다. **알려진 정합성 결함은 현재 없다.**
> 남은 것은 복구 장치(역방향 대사) 하나와 문서다. 마스터플랜 어필 체크리스트 11개 중 미확보는
> F-3의 "정산의 법적 맥락·README" 하나뿐 — **기술 축은 사실상 다 쌓였다.**
0. **(선행) ADR-020 결론부 검토** — 상태란이 "작성자 검토 대기". 초안 판단이 본인 결론과 맞는지 확인.
...
5. 나머지는 결제/대기열/기타 백로그 — 실측 계열은 인프라 의존. 프론트 401 인터셉터(~10줄)는 기타 백로그.
