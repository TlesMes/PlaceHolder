...
다만 예측의 절반은 빗나갔다 — "제공자 수가 무관해진다"고 적었는데 **1.5배가 남았다**(4.0배에서 줄었을 뿐).
2회차까지는 사라진 줄 알았다가 5회로 늘리자 뒤집혔다. **병목이 사라지자 회차 간 변동이 8% → 41%로 커져,
적은 회차로는 1.5배를 가려낼 수 없었다** — 병목이 값을 붙잡아 주고 있었던 것이다.
잔여의 원인은 [미결로 남겼다](docs/performance/provider-settlement-throughput.md).
