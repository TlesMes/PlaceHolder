...
- **Phase D-3: 제공자 정산 처리량 측정** — PR #31 (`feature/settlement-throughput-measure`, **머지 완료 2026-08-22**)
  ...
  - **판정: 병목이 맞다.** `1/락보유시간`으로 유도한 상한(91·83건/초)과 실측 P=1 처리량(88.1/81.2)이 **오차 4% 안에서 일치** — 부하를 민 값이 아니라 다른 경로로 계산한 값이 실측과 만나 **원인 귀속이 닫혔다**(D-2에 없던 내부 지표). P=1에서 지연의 **93%가 락 대기**이고 P가 커질수록 줄어드는 건 대기뿐(303→27ms). 제공자당 ≈85건/초로 요구 기준선 미달, 박스 용량(250–350)의 1/3–1/4
...
- **Phase D-3: 정산 잔액을 거래 내역 합계로 계산** — PR #32 (`feature/settlement-derive-from-ledger`, **머지 완료 2026-08-22**, ADR-021)
  - #31이 병목으로 판정한 제공자 계정 행을 **없앴다.** 확정은 `SETTLE` 행만 INSERT하고, 잔액은 조회 시 그 합으로 계산한다. `settlement_balance` 컬럼·`settle()`·`findByUserIdForUpdate` 삭제
  - **왜 락이 아니라 컬럼인가:** #31의 비교군 ②(락만 제거)가 1.4배에 그쳤다. `FOR UPDATE`를 빼도 `UPDATE settlement_balance`가 남아 커밋 시점에 같은 행이 다시 잠기고, 락 보유 11ms 자체가 대부분 커밋 시간이었다(락을 트랜잭션 마지막에 잡으므로 그 뒤로 flush·commit뿐) → **트랜잭션 안에 그 행 쓰기가 있는 한 커밋 시간만큼은 대기가 생긴다.** 원자적 `SET balance = balance + ?`도 여기서 탈락
  - **결과(P=1, 인기 이벤트 상황):** 지연 p50 347→92ms(3.8배)·p95 478→142·p99 496→165, 처리량 85→304건/초(3.6배, 요구 기준선 100건/초의 3배). ⚠️ **둘은 독립 증거가 아니다** — 스레드 32 고정 폐쇄 루프라 리틀의 법칙으로 묶여 있고(32/지연≈처리량, 10% 내 일치) 같은 사실의 두 표현이다
  - **직렬화 제거는 측정이 아니라 구조가 보증한다** — 잠글 컬럼이 존재하지 않는다. lost update도 마찬가지로 **구조적으로 불가능**해졌다(#28의 락은 동시 갱신을 막는 방식이라 락이 빠지면 결함이 돌아오지만, 지금은 덮어쓸 대상 자체가 없다). 과거 유실분도 자동 복구된다(이력은 온전했으므로). 락 순서 규약 3→2단계(좌석 → 예약자 계정)
  - **⚠️ 예측 절반은 빗나갔고, 그 과정에서 내 판정이 한 번 틀렸다:** "제공자 수 무관(≈1.0배)" 예측에 대해 **1.5배가 남았다.** 2회 데이터로 "차이가 사라졌다"고 판단했다가 **5회에서 뒤집혔다**(중앙값 304/382/436/448 단조 증가, 순위합 U=2). 원인은 **병목이 사라지자 회차 간 변동이 8%→41%로 커진 것** — 락이 속도를 정할 때는 값이 붙잡혀 있었다. **측정이 안정적이었던 것이 좋은 신호가 아니라 병목의 증상이었다.** 판정 기준을 1회 측정으로 설계한 것이 부실했음
  - 같은 이유로 **#31의 P=8(352건/초)을 "이 PC의 한계"로 읽은 것도 틀렸다** — 그때도 제공자당 30건(240÷8)씩 대기가 있었다. 파생 후 최고 518건/초
  - **남은 1.5배는 제거한 락의 잔여가 아니다**(락이 없다). 4배 효과에 가려 있던 별개 현상. 원인 미확정 — 유력 후보는 ①인덱스 삽입 핫스팟(`SETTLE` 240행이 같은 `user_id`라 같은 리프 페이지) ②FK 부모(`users`) 같은 행 S락 ③**하네스 교란**(픽스처가 제공자 1명당 이벤트 1개라 제공자 수와 이벤트 수를 못 가른다 → 정확히는 "이 픽스처에서 P를 줄이면 느리다"까지만 말할 수 있음). 인덱스 제거 진단 2회는 **결론 불가**(기존 `idx_pt_user_id`가 남아 같은 클러스터링이 유지됨)
  - **심각도 낮음, 착수 안 함:** 옛것은 커밋까지 잡는 행 락이라 **트랜잭션 길이가 곧 대기**였지만(로직 추가 시 천장이 같이 하락), 새것은 페이지 래치라 그 연결이 없다. auto-increment PK가 이미 겪는 것과 같은 종류. 쫓게 되면 첫 순서는 DB 내부가 아니라 **픽스처에서 제공자 수/이벤트 수 분리**
  - 커버링 인덱스 `idx_pt_settlement_sum (user_id, type, created_at, amount)`. `created_at`은 오늘 안 쓰지만(= 조건이 앞이라 성능 동일) **스냅샷·정산 페이징이 같은 인덱스를 쓰게 하는 의도적 헤지** — 인덱스 컬럼 순서는 호출부에 노출되는 표면이 아니라 PR #29가 걷어낸 "쓰지 않는 유연성"과 다름
  - **스냅샷 유보:** `PAYOUT` 생기면 도메인상 자연히 유계 / 조회 O(행수)는 무페이징 기존 부채와 같은 축 / **경계가 위험**(`createdAt`은 `@PrePersist` = 커밋 시각이 아니라 인서트 시각 → 늦게 시작해 먼저 커밋한 트랜잭션이 빠지거나 두 번 세어짐, 결제 보정 잡의 `now-안전여유` 패턴 필요) / 나중에 넣어도 재작업은 쿼리 1 + 메서드 ~5줄
  - 테스트 208 통과. ⚠️ `ProviderSettlementConcurrencyTest`는 **계획과 달리 한 줄 수정 필요**(`getSettlementBalance()`가 사라져 컴파일 불가). 그 단정의 의미도 바뀌었다 — 예전엔 lost update 탐지기였으나 지금은 갈라질 두 곳이 없어 "이력이 온전한가"에 가깝다. **싱거워진 것 자체가 결론**이라 주석에 남김
  - **문서 톤 교정(작성자 지적 2회):** ①PR 제목 "정산 잔액을 원장에서 파생 — 없앨 것은 락이 아니라 쓰기였다"가 비직관적 → **작업 위주** "정산 잔액을 컬럼 대신 거래 내역 합계로 계산"(ADR 제목도 동일 기조). 본문도 슬로건형 인용구·과다 볼드·비유를 걷어내고 평서문으로 ②전후 비교가 P 스윕 표에 묻힘 → **전/후 지연·처리량을 먼저**, P 스윕은 "원인 귀속용 진단 축"으로 후순위

- **좌석 상태 변경 시 바뀐 값만 UPDATE** — PR #33 (`refactor/seat-dynamic-update`, **머지 완료 2026-08-22**)
  - #32가 "원인 미상"으로 남긴 1.5배를 추적해 해소(절반). **정산과 무관한 별개 건** — 좌석 UPDATE는 홀드·확정·반환·만료가 전부 지나가는 경로라 이득도 거기 전체에 걸린다
  - **먼저 한 일은 DB를 뒤지는 게 아니라 픽스처를 고치는 것이었다.** 하네스가 제공자 1명당 이벤트 1개를 만들어 두 수가 붙어 움직였다(#32에 한계로 명시해둔 그 지점). 이벤트를 라운드로빈 배정해 분리하니 **(1,1) 266 / (1,8) 392 / (8,8) 392건/초** — 이벤트만 흩으면 제공자가 몰려도 빨라지고 거기서 제공자를 더 흩어도 무변화. **원인은 제공자가 아니라 이벤트.** #32에서 세운 후보 중 제공자에 걸린 둘(`user_id` 인덱스 핫스팟, `users` FK S락)이 이 측정으로 탈락
  - **원인:** Hibernate 기본값은 엔티티마다 UPDATE 문을 하나 만들어 재사용하는 것이라 **안 바뀐 칸도 전부 쓴다.** 확정은 `status`·`held_by`·`held_until` 3칸만 바꾸는데 SQL은 6칸(`event_id`·`label`·`price` 포함). 그중 `event_id`·`label`이 `uk_seats_event_label`을 이루고 이 인덱스는 `event_id` 순 정렬 → **한 이벤트의 좌석들이 인덱스 같은 자리에 모여** 동시 확정이 매번 거기를 거친다. 인기 이벤트가 정확히 그 모양
  - **어느 쪽도 잘못이 아니다** — 문장 고정(캐시 재사용)도, "같은 이벤트에 같은 좌석 이름 금지" 유니크 제약도 각각 옳다. 겹치는 지점에서, 좌석이 한 이벤트에 몰렸을 때만 값이 나온다
  - 조치: `Seat`에 `@DynamicUpdate`(인덱스는 도메인 규칙이라 유지, 헛되이 들르지만 않게). **266 → 323건/초(+21%), 흩어진 칸은 무변화**(392→408/385) — 예측 방향과 일치. 격차 1.47→1.25배
  - **⚠️ 코드만 읽어서는 못 찾는 종류다.** 우리 코드엔 `seat.confirm()`뿐이고 3필드만 바꾼다. 6칸을 쓴다는 건 프레임워크가 만드는 것이라 **SQL을 찍어봐야 보인다**(`-Dspring.jpa.show-sql=true`)
  - 남긴 것: 격차 1.25배는 여전히 이벤트에 걸려 있으나 미특정(확정마다 같은 이벤트 행 읽기가 후보) / DB 내부의 어느 단계가 비용인지는 미확정(InnoDB가 값 무변경 시 보조 인덱스 갱신을 건너뛰기도 함) / 정합성 무관·기준선 3배라 미착수. 상세: `docs/performance/provider-settlement-throughput.md` 9절
  - 테스트 208 통과(동작 변경 없어 신규 없음). ⚠️ 커밋 제목은 옛 문구(`좌석 UPDATE에서 안 바뀐 컬럼 제외`) — 비직관적이라 PR 제목만 교체, 히스토리는 미수정

- **프론트 401 응답 인터셉터** — PR #34 (`fix/frontend-401-interceptor`, **머지 완료 2026-08-22**)
  - 요청 인터셉터(토큰 부착)만 있고 응답 인터셉터가 없어 서버가 401을 줘도 토큰이 남았다 → **"화면은 로그인, 서버는 미인증"** 유지. `AuthContext`는 `exp`를 **마운트 시점에만** 검사해 만료와 무관한 거부 사유를 못 잡는다(①토큰 만료 ②soft delete된 사용자 ③JWT 시크릿 교체)
  - 401이면 토큰 삭제 + `window.location.replace('/login')`. **하드 내비게이션이라 `AuthContext`가 토큰 없는 상태로 다시 읽힌다** — React 상태를 따로 건드릴 필요 없음
  - **⚠️ 핵심 함정 — 로그인·회원가입 요청 제외:** 이쪽 401은 "자격 증명이 틀렸다"는 뜻이라 화면이 직접 안내해야 한다. 함께 처리하면 로그인 실패 시 **에러 메시지를 볼 새도 없이 화면이 갈아엎힌다.** 인터셉터를 넣을 때 가장 흔한 사고라 실측으로 확인함(로그인 실패가 실제로 401로 온다)
  - **403은 건드리지 않는다** — 이 프로젝트는 401/403 의미를 분리해뒀고 조건은 `status === 401`. 테스트가 아니라 **조건문이 보장**하는 것이라, 나중에 "403도 잡자"로 넓히면 역할 거부가 곧 로그아웃이 된다
  - **브라우저 E2E 5종(2개 세션 분담):** 틀린 비번→화면 유지 / 정상 로그인→200 / **서명 조작 토큰**→401→삭제+리다이렉트(= JWT 시크릿 교체 경로) / **사라진 계정의 잔존 토큰**→동일(= soft delete·DB 재생성 경로) / **양성 대조: 유효 토큰→200 정상 로드, 오탐 리다이렉트 없음**. 되풀이 이동 없음(`/login` 도착 후 백엔드 요청 정지) 확인
  - 프론트 파일 1개, 백엔드 무변경. 재현 경로 3개 중 2개가 실증됨(나머지 만료는 `AuthContext`가 이미 잡는 경로)

### 현재 상태
- **작업 브랜치:** `main` (**D-3 측정→재설계→좌석 UPDATE 후속 + 401 인터셉터 완료**, 다음은 E-4 미착수)
- **마지막 main 커밋:** `Merge pull request #34` (f776a42)
  - PR #1–11, #13–34 머지 완료
...
> 취소의 크래시 창은 #30(역방향 대사)으로, #28이 남긴 직렬화 상한은 #31 측정 → #32 재설계로 닫혔다.
> **알려진 정합성 결함·미복구 경로가 현재 없다.** 어필 체크리스트 11개도 F-3로 전부 확보됐다.
> 남은 것은 미착수 Phase(E-4 분산, F-1 CD)와 인프라 의존 실측이다.
...
3. ~~**제공자 정산 처리량 측정 → 파생 재설계**~~ → **PR #31·#32 머지 완료 2026-08-22.** 측정으로 병목 판정(1/락보유시간 ≈ 실측, 오차 4%) → 잔액 컬럼 제거로 원인 제거(지연 347→92ms, 처리량 85→304건/초). **D-1(N+1)에 이어 측정이 설계를 바꾼 두 번째 사례.** 남은 1.5배는 **PR #33에서 추적 — 원인은 제공자가 아니라 이벤트**(좌석 UPDATE가 안 바뀐 인덱스 컬럼을 쓰고 있었다). 절반 해소, 나머지 1.25배는 미착수
...
5. ~~**충전 한도·감사 로그·영수증** 없음 — 실제 요건 확인 필요~~ → **조사 완료 2026-08-22.** 포인트가 제3자(제공자)에게 쓰이므로 정의상 선불전자지급수단에 해당하나, 등록 면제 구간(발행잔액 30억/연 500억 미만)이고 실서비스가 아니라 등록·선불충전금 별도 관리 의무는 무관. **대신 기능 단위 결함 1건을 찾아 수정**: 취소 기한 7일이 청약철회권(거래를 물릴 권리)과 미사용 잔액 환급권(안 쓴 돈을 돌려받을 권리)을 한 기한으로 묶어, `PAYOUT` 경로가 없는 구조에서 "8일째부터 미사용 현금 영구 인출 불가"가 되고 있었다 → **카드 취소 실질 상한 1년으로 개정**(ADR-019 1-1). 5년이 아닌 것은 카드사 보관 기간을 넘으면 토스 취소가 실제로 실패해 **지킬 수 없는 약속**이 되기 때문. 출금(`PAYOUT`)·환급률(90%) 정책은 실서비스 사업자 의무 영역이라 **범위 밖으로 확정**(미루는 게 아니라 안 함). ⚠️ 무단 수수료 차감류는 애초에 없음 — 환불액은 `min(주문 잔여액, 유료 잔액)`뿐이고 퍼센트 차감 로직 자체가 코드에 없다.
