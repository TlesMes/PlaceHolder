...
5. **E-4 분산 환경 동시성**(분산 락) — 미착수. ⚠️ **2026-08-22 검토 결과 가치 낮다고 판단:** ①입장 상한은 `admit.lua` 원자성이라 인스턴스 수와 무관(코드 논증으로 종결) ②deficit 장부 인메모리 한계는 이미 알고 문서화됨(재확인해도 새 정보 없음) ③대사 잡 중복 실행만 검증 가치가 있는데 **실 인스턴스 2개가 아니라 D-3식 인프로세스 동시 호출로 충분** ④DB락 vs Redis락 비교는 Redisson 의존성부터 없어 사실상 **새 기능 개발 + ADR**(검증 세션이 아님). 착수하려면 ③만 떼어 테스트 1건으로 하는 게 비용 대비 낫다
