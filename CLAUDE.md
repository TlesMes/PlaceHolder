...
- **마지막 main 커밋:** `Merge pull request #26` (bc51a4e)
  - PR #1~11, #13~26 머지 완료
...
### 다음 작업 (우선순위 순)
> E-2 결제가 생애주기 축까지 완결됐다(#26). 다만 **#26 self-review에서 금전적 구멍 1건이 확인돼** 그것이 최우선이다. 마스터플랜 어필 체크리스트 11개 중 미확보는 F-3의 "정산의 법적 맥락·README" 하나.
1. **무료/유료 포인트 구분 (최우선, 정책 확정됨 2026-08-19)** — 쿠폰 상환도 `CHARGE`로 기록돼 잔액에서 현금 충전과 구분되지 않는다. 그래서 `현금충전 → 좌석에 소진 → 쿠폰 상환 → 결제 취소` 순서로 **쿠폰이 현금으로 환전**된다(좌석은 그대로 남음). 정확도가 아니라 **구멍**. **확정: 잔액을 무료(쿠폰)/유료(현금) 버킷으로 구분 기록, 소모는 무료 우선, 환불은 유료에서만.** 충전 건별 귀속은 범위 밖(현금↔현금이라 무손실). ⚠️ 좌석 확정 트랜잭션(C-2)을 건드리므로 별도 검증 필수.
2. **역방향 대사 (PR-B)** — #26이 만든 크래시 창(①커밋 후 ②토스 호출 전 사망 → 포인트만 회수되고 환불 안 됨)을 메운다. 후보는 `cancel_confirmed_at IS NULL`인 취소 주문 = "다시 보내야 할 요청". 멱등 키가 있어 재시도가 안전하다. **1번이 구멍이고 이쪽은 복구 장치라 순서상 1번이 먼저.**
3. **F-3 포트폴리오 문서 마감** — README 확장(domain→problem→solution→result), ADR 19종 정리, 면접 스토리 2종(동시성 설계 서사 / 정산의 법적 맥락). **인프라 의존 없음.**
4. **E-4 분산 환경 동시성**(분산 락) — 마스터플랜 미착수 Phase. 멀티 인스턴스+LB 전제라 *인프라 의존*.
5. **F-1 CD**(AWS EC2 배포) — CI는 #12로 완료, 배포는 미착수. 마스터플랜상 "시간 여유 시".
6. 나머지는 결제/대기열 백로그 — 실측 계열은 인프라 의존.
...
1. **환불 재원 구분 없음 (→ "다음 작업" 1번으로 승격)** — 쿠폰 상환도 `CHARGE`라 잔액에서 현금 충전과 구분 불가 → 쿠폰이 현금으로 환전되는 경로가 열려 있다. 무료/유료 버킷 분리로 해소 예정. `PointTransaction`에 `PaymentOrder` 연결이 없다는 기존 "구조적 공백"도 이때 함께 볼 지점(다만 건별 귀속은 손실이 없어 필수는 아님).
