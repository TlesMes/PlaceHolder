...
### 현재 상태
- **작업 브랜치:** `main` (**정산 처리량 측정 → 재설계 완료**, 다음 작업 미착수)
- **마지막 main 커밋:** `Merge pull request #32` (ed06da8)
  - PR #1–11, #13–32 머지 완료
...
3. ~~**제공자 정산 처리량 측정 → 파생 재설계**~~ → **PR #31·#32 머지 완료 2026-08-22.** 측정으로 병목 판정(1/락보유시간 ≈ 실측, 오차 4%) → 잔액 컬럼 제거로 원인 제거(지연 347→92ms, 처리량 85→304건/초). **D-1(N+1)에 이어 측정이 설계를 바꾼 두 번째 사례.** 남은 1.5배는 락과 무관한 별개 현상이고 심각도 낮아 미착수(위 PR #32 항목 참조)
