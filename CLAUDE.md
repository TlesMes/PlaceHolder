...
1. **취소·환불 부재(가장 큰 구멍)** — 토스 취소 API 호출 + 포인트 회수 경로가 0. 실서비스면 청약철회가 법적 요구라 필수. "이미 써버린 포인트는 어떻게 되나"(잔액 부족 시 회수 정책·부분 취소)라는 도메인 설계가 따라온다. **보상 트랜잭션 문제**라 결제보다 어렵다 — 결제는 외부 성공+내부 실패 시 재시도로 복구되지만(멱등), 취소는 "돈은 돌려줬는데 포인트 회수 실패"가 순손실이다. 대사(#24)가 그 안전망 역할을 하므로 순서상 지금이 착수 적기.
